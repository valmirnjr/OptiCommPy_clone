...
@njit
def compute_min_dist_to_constellation_symbols(symbol, const_symb):
    dist_to_each_const_symbol = np.abs(symbol - const_symb) ** 2
    return np.min(dist_to_each_const_symbol)
...
def bps(Ei, N, constSymb, B):
    ...
    for n in range(0, nModes):
        ...
        for symbol_index in range(0, num_symbols_after_padding):
            symbol = x[symbol_index, n]
            for indPhase, ϕ in enumerate(ϕ_test):
                rotated_symbol = symbol * np.exp(1j * ϕ)
                dmin[indPhase, -1] = compute_min_dist_to_constellation_symbols(rotated_symbol, constSymb)
            if symbol_index >= 2 * N:
                θ[symbol_index - 2 * N, n] = get_optimum_phase_angle(ϕ_test, dmin)
            dmin = np.roll(dmin, -1)
    return θ
